                ...
                <div className="px-3 md:px-4 pt-2 md:pt-2 border-black dark:border-white border-[1px] rounded-lg md:rounded-xl h-[100%]">
                    ...
                    <div className="text-center text-lg grid grid-cols-2 gap-2">
                        ...
                        {dateDiff > 0 && <div className='grid gap-0 border-[1px] border-dotted border-black dark:border-white p-2'>
                            <span className='font-light uppercase text-sm leading-[1rem]'>Days Until Event:</span>
                            <span className={`text-3xl ${orbitron.className} text-center p-0 leading-[1.75rem]`}>{dateDiff}</span>
                        </div>}

                        {/* EVENT URL */}
                        {data.event_url && <a href={data.event_url} target="_blank" className='inline-grid justify-center items-center text-2xl border-[1px] border-dotted border-black dark:border-white p-2 leading-[1rem] bg-blue-400 dark:text-black hover:bg-blue-800 hover:text-white transition-all'>
                        ...
                        {finishTime.epoch > 0 && <a href={data.race_stats.results_url} target="_blank" className='grid gap-0 border-[1px] border-dotted border-black dark:border-white p-2 bg-blue-400 dark:text-black hover:bg-blue-800 hover:text-white transition-all'>
                            <span className='font-light uppercase text-sm leading-[1rem]'>Finish Time</span>
                            <span className={`text-3xl ${orbitron.className} text-center p-0 leading-[1.75rem]`}>{finishTime.hour}:{finishTime.minutes}:{finishTime.sec}</span>
                        </a>}

                        {/* BIB */}
                        {finishTime.epoch > 0 && <div className='grid gap-0 border-[1px] border-dotted border-black dark:border-white p-2'>
                            <span className='font-light uppercase text-sm leading-[1rem]'>Bib Number</span>
                            <span className={`text-3xl ${orbitron.className} text-center p-0 leading-[1.75rem]`}>{data.race_stats.bib || 'virtual'}</span>
                        </div>}
